...
func (rq *RoomQuery) QueryBeds() *BedQuery {
	query := (&BedClient{config: rq.config}).Query()
...
func (rq *RoomQuery) QueryWard() *WardQuery {
	query := (&WardClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		...
		withBeds:   rq.withBeds.Clone(),
		withWard:   rq.withWard.Clone(),
...
func (rq *RoomQuery) WithBeds(opts ...func(*BedQuery)) *RoomQuery {
	query := (&BedClient{config: rq.config}).Query()
...
func (rq *RoomQuery) WithWard(opts ...func(*WardQuery)) *RoomQuery {
	query := (&WardClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withBeds; query != nil {
		if err := rq.loadBeds(ctx, query, nodes,
			func(n *Room) { n.Edges.Beds = []*Bed{} },
			func(n *Room, e *Bed) { n.Edges.Beds = append(n.Edges.Beds, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withWard; query != nil {
		if err := rq.loadWard(ctx, query, nodes, nil,
			func(n *Room, e *Ward) { n.Edges.Ward = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadBeds(ctx context.Context, query *BedQuery, nodes []*Room, init func(*Room), assign func(*Room, *Bed)) error {
	...
	query.Where(predicate.Bed(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.BedsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.room_beds
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_beds" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "room_beds" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (rq *RoomQuery) loadWard(ctx context.Context, query *WardQuery, nodes []*Room, init func(*Room), assign func(*Room, *Ward)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Room)
	for i := range nodes {
		if nodes[i].ward_rooms == nil {
			continue
		}
		fk := *nodes[i].ward_rooms
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ward_rooms" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
