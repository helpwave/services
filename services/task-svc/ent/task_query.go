...
func (tq *TaskQuery) QueryPatient() *PatientQuery {
...
func (tq *TaskQuery) QuerySubtasks() *SubTaskQuery {
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withPatient:  tq.withPatient.Clone(),
...
func (tq *TaskQuery) WithPatient(opts ...func(*PatientQuery)) *TaskQuery {
	query := (&PatientClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withPatient = query
	return tq
}
...
func (tq *TaskQuery) WithSubtasks(opts ...func(*SubTaskQuery)) *TaskQuery {
	query := (&SubTaskClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withPatient; query != nil {
		if err := tq.loadPatient(ctx, query, nodes, nil,
			func(n *Task, e *Patient) { n.Edges.Patient = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withSubtasks; query != nil {
		if err := tq.loadSubtasks(ctx, query, nodes,
			func(n *Task) { n.Edges.Subtasks = []*SubTask{} },
			func(n *Task, e *SubTask) { n.Edges.Subtasks = append(n.Edges.Subtasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadPatient(ctx context.Context, query *PatientQuery, nodes []*Task, init func(*Task), assign func(*Task, *Patient)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		if nodes[i].patient_tasks == nil {
			continue
		}
		fk := *nodes[i].patient_tasks
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "patient_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TaskQuery) loadSubtasks(ctx context.Context, query *SubTaskQuery, nodes []*Task, init func(*Task), assign func(*Task, *SubTask)) error {
